...
<?php $globalButtons=$this->var['globalButtons'] = !empty($group->desc) ? json_decode($group->desc, true) : array();?>
...
<?php if($slides){ ?>
  ...
  <div class='block<?php echo $blockClass;?>' id='block<?php echo $block->id;?>'>
    ...
    <?php foreach($slides as $slide): ?>
      ...
      <?php $target=$this->var['target'] = " data-target='" . (isset($slide->target) && $slide->target ? '_blank' : '_self') . "'";?>
...
<?php } ?>
      ...
      <?php if($slide->backgroundType == 'image'){ ?>
        ...
        <div class='item<?php echo ($index === 0) ? ' active' : ''; ?>' <?php echo $url . ' ' . $target; ?>>
        <?php print(html::image($slide->image)); ?>

      <?php }else{ ?>

        <div class='item<?php echo ($index === 0) ? ' active' : ''; ?>' <?php echo $url . ' ' . $target; ?> style='<?php echo 'background-color: ' . $slide->backgroundColor . '; height: ' . $height . 'px'; ?>'>
      <?php } ?>
...
<?php echo html::a($slides["$id"]->buttonUrl["$key"], $slides["$id"]->label["$key"], "class='btn btn-{$slides["$id"]->buttonClass["$key"]}' target='{$slides["$id"]->buttonTarget["$key"]}'"); ?>

<?php } ?>

                <?php if(!$slides["$id"]->buttonUrl["$key"]){ ?>

<?php echo html::commonButton($slides["$id"]->label["$key"], "btn btn-{$slides["$id"]->buttonClass["$key"]}"); ?>
...
<?php echo html::a($slide->buttonUrl["$key"], $label, "class='btn btn-{$slide->buttonClass["$key"]}' target='{$slide->buttonTarget["$key"]}'"); ?>

<?php } ?>

              <?php if(!$slide->buttonUrl["$key"]){ ?>

<?php echo html::commonButton($label, "btn {$slide->buttonClass["$key"]}"); ?>
